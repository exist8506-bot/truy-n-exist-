...
const storageGetRaw=(key,fallback='')=>{try{return localStorage.getItem(key)??fallback}catch{return fallback}};
const storageRemove=(key)=>{try{localStorage.removeItem(key)}catch{}};
const readerFont=(()=>{const f=storageGetRaw('ktf_font');return f==='Georgia'||f==='system-ui'?f:'system-ui'})();
const safeNum=(v,d,min,max)=>{const n=Number(v);return Number.isFinite(n)?Math.max(min,Math.min(max,n)):d};const safeTheme=(v)=>['dark','light','sepia'].includes(v)?v:'dark';const safeLang=(v)=>Object.prototype.hasOwnProperty.call(I18N||{},v)?v:'vi';
 const state={books:[],book:null,chapter:0,chapterMin:0,chapterMax:-1,chapters:[],remoteSearch:false,apiAvailable:false,order:'asc',filter:'all',sort:'title',mode:'all',libraryPage:1,libraryPageSize:100,libraryTotal:0,fontSize:safeNum(storageGetRaw('ktf_fs'),19,14,30),font:readerFont,theme:safeTheme(storageGetRaw('ktf_theme')),rate:safeNum(storageGetRaw('ktf_rate'),1,0.5,2),continuous:false,lang:['vi','en','zh'].includes(storageGetRaw('ktf_lang'))?storageGetRaw('ktf_lang'):'vi',user:null,token:storageGetRaw('ktf_token')||'',offline:new Set()};
const I18N={
...
window.configureApi=()=>{const current=window.KhoAPI?.base?.()||'';const value=prompt('URL API backend (ví dụ: https://api.example.com/api/v1)\\nĐể trống để dùng mặc định của trang.',current);if(value===null)return;const v=value.trim().replace(/\/$/,'');if(v)save('ktf_api_base',v);else storageRemove('ktf_api_base');toast(v?'Đã lưu địa chỉ API. Đang tải lại…':'Đã khôi phục API mặc định. Đang tải lại…');setTimeout(()=>location.reload(),350)};
...
function save(k,v){try{localStorage.setItem(k,typeof v==='string'?v:JSON.stringify(v));return true}catch(e){console.warn('localStorage save failed',k,e?.message||e);return false}}
function get(k,d){try{const raw=localStorage.getItem(k);if(raw==null)return d;try{return JSON.parse(raw)}catch{return raw}}catch(e){console.warn('localStorage read failed',k,e?.message||e);return d}}
function favs(){const v=get('ktf_favs',[]);return Array.isArray(v)?[...new Set(v.map(x=>String(x||'').trim()).filter(Boolean))]:[]} function setFavs(v){save('ktf_favs',v)}
...
async function openBook(id){
 const openId=++bookOpenId;show('detail');let b=state.books.find(x=>x.id===id);
 if(state.apiAvailable){
  try{b=normalizeBook(await api('/stories/'+encodeURIComponent(id)))}catch{}
 }
 if(openId!==bookOpenId)return null;
 ...
 await loadChapterPage(1);if(openId!==bookOpenId)return null;
 return b;
}
...
function scheduleAutoAdvance(){
 ...
 autoAdvanceTimer=setTimeout(async()=>{
  ...
  const opened=await readChapter(next);
  if(!opened&&keepTts&&runId===ttsRunId){stopTTS(true);return}
  if(opened&&keepTts&&runId===ttsRunId)startTTSCurrent();
 },900);
}
async function readChapter(i,options={}){
 ...
 const n=totalChapterCount();
  if(i<state.chapterMin||i>state.chapterMax||state.chapterMax<state.chapterMin)return false;
...
function bookmarks(){try{const v=JSON.parse(localStorage.getItem('ktf_bookmarks_v1')||'{}');return v&&typeof v==='object'&&!Array.isArray(v)?v:{}}catch{return{}}}
function setBookmarks(v){save('ktf_bookmarks_v1',v&&typeof v==='object'&&!Array.isArray(v)?v:{})}
...
function renderBookmarks(){const box=$('#bookmarkList');if(!box)return;const b=bookmarks(),items=[];for(const [storyId,chs] of Object.entries(b)){const story=state.books.find(x=>x.id===storyId);for(const [idx,v] of Object.entries(chs||{})){const index=Number(idx),display=Number.isInteger(index)?(story?.chapterMin===0?index+1:index):(story?.chapterMin??index);items.push({storyId,index,title:v?.title||('Chương '+display),at:Number(v?.at||0),storyTitle:story?.title||storyId,display})}}items.sort((a,b)=>b.at-a.at);box.innerHTML=items.length?items.map(x=>'<div class="histrow"><div><b>'+esc(x.storyTitle)+'</b><div class="muted">'+T('chapterUnit')+' '+x.display+' · '+esc(x.title)+'</div></div><button class="btn" onclick="openBook(\''+esc(x.storyId)+'\').then(()=>readChapter('+x.index+'))">'+T('read')+'</button></div>').join(''):'<div class="empty">'+T('noBookmarks')+'</div>'}
window.goChapter=d=>{const x=state.chapter+d;if(x<state.chapterMin||x>state.chapterMax){toast(d<0?T('prev'):T('next'));return}const keepTts=ttsState.active,runId=ttsRunId;if(keepTts)try{speechSynthesis?.cancel?.()}catch{}readChapter(x).then(opened=>{if(!opened&&keepTts&&runId===ttsRunId){stopTTS(true);return}if(opened&&keepTts&&runId===ttsRunId)startTTSCurrent()})};
window.font=d=>{state.fontSize=Math.max(14,Math.min(30,state.fontSize+d));save('ktf_fs',state.fontSize);applyReader()};window.setFont=x=>{state.font=x==='Georgia'||x==='system-ui'?x:'system-ui';save('ktf_font',state.font);applyReader()};window.setTheme=x=>{state.theme=safeTheme(x);save('ktf_theme',state.theme);applyReader()};let ttsState={active:false,chunks:[],pos:0,chapterKey:'',mode:''};
...
function speakTTSChunk(runId=ttsRunId){
 ...
 if(ttsState.pos>=ttsState.chunks.length){
   ...
   const keep=runId;
   try{speechSynthesis?.cancel?.()}catch{}
    readChapter(next).then(opened=>{if(!opened&&keep===ttsRunId){stopTTS(true);return}if(opened&&keep===ttsRunId&&ttsState.active)startTTSCurrent()});
  }else stopTTS(true);
  return;
 }
...
window.setTTSRate=x=>{state.rate=safeNum(x,1,0.5,2);save('ktf_rate',state.rate);if(ttsState.active){stopTTS(true);startTTSCurrent()}};
...
function updateScrollUi(pct){if(!state.book)return;$('#rprogress').style.width=Math.min(100,pct)+'%';$('#readPosition').textContent=T('chapterUnit')+' '+chapterDisplayNumber(state.chapter)+' / '+(state.book.chapterCount||1)+' · '+Math.round(pct)+'%'}
...
function renderHistory(){const h=hist();$('#historyList').innerHTML=h.length?h.map(x=>{const b=state.books.find(b=>b.id===x.bookId),cn=Number(x.chapter);const display=Number.isInteger(cn)?(b?.chapterMin===0?cn+1:cn):(b?.chapterMin??'');return '<div class="histrow"><div><b>'+esc(b?.title||x.bookId)+'</b><div class="muted">'+T('chapterUnit')+' '+display+' · '+esc(x.title)+'</div></div><button class="btn" onclick="openBook(\''+esc(x.bookId)+'\').then(()=>readChapter('+x.chapter+'))">'+T('read')+'</button></div>'}).join(''):'<div class="empty">'+T('noHistory')+'</div>'}
...
window.logout=async()=>{try{await api('/auth/logout',{method:'POST'})}catch{}state.token='';storageRemove('ktf_token');state.user=null;updateStats();toast('Đã đăng xuất');closeAccount()};
...
s.querySelector('#adRestoreBtn').onclick=async()=>{const f=s.querySelector('#adRestore').files[0];if(!f)return out.textContent='Chọn file backup JSON';if(!confirm('Restore sẽ thay toàn bộ dữ liệu hiện tại. Tiếp tục?'))return;try{const j=JSON.parse(await f.text());const result=await api('/admin/backup/restore',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(j)});if(result?.token){state.token=result.token;save('ktf_token',result.token)}else{state.token='';state.user=null;storageRemove('ktf_token')}out.textContent='Restore thành công. Đang tải lại...';location.reload()}catch(e){out.textContent='Restore lỗi: '+e.message}};
