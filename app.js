...
const safeNum=(v,d,min,max)=>{const n=Number(v);return Number.isFinite(n)?Math.max(min,Math.min(max,n)):d};const safeTheme=(v)=>['dark','light','sepia'].includes(v)?v:'dark';const safeLang=(v)=>Object.prototype.hasOwnProperty.call(I18N||{},v)?v:'vi';
 const state={books:[],book:null,chapter:0,chapterMin:0,chapterMax:-1,chapters:[],remoteSearch:false,apiAvailable:false,order:'asc',filter:'all',sort:'title',mode:'all',libraryPage:1,libraryPageSize:100,libraryTotal:0,fontSize:safeNum(localStorage.getItem('ktf_fs'),19,14,30),font:readerFont,theme:safeTheme(localStorage.getItem('ktf_theme')),rate:safeNum(localStorage.getItem('ktf_rate'),1,0.5,2),continuous:false,lang:['vi','en','zh'].includes(localStorage.getItem('ktf_lang'))?localStorage.getItem('ktf_lang'):'vi',user:null,token:localStorage.getItem('ktf_token')||'',offline:new Set()};
const I18N={
