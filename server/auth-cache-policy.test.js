...
(async()=>{try{
 ...
 const etag=personal.headers.get('etag');
 const conditional=await fetch('http://127.0.0.1:8787/api/v1/progress',{headers:{...auth,'if-none-match':etag||'"no-match"'}});
 if(conditional.status!==200)throw Error('authenticated conditional GET returned '+conditional.status);
 await conditional.json();
