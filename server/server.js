...
if(p==='/api/v1/stories'&&req.method==='GET'){const ck=cacheKey(req),hit=cacheGet(ck);if(hit)return reply(200,hit);let where=[],args=[],sql=`SELECT s.*,COUNT(c.id) chapters FROM stories s LEFT JOIN chapters c ON c.story_id=s.id`;const qv=String(u.query.q||'').trim().normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();if(qv){where.push(`s.search_key LIKE ?`);const z='%'+norm(qv)+'%';args.push(z)}if(u.query.author&&u.query.author!=='all'){where.push('s.author=?');args.push(String(u.query.author))}if(u.query.category&&u.query.category!=='all'){where.push('s.category=?');args.push(String(u.query.category))}if(u.query.status&&u.query.status!=='all'){where.push('s.status=?');args.push(String(u.query.status).toUpperCase())}if(where.length)sql+=' WHERE '+where.join(' AND ');sql+=' GROUP BY s.id';const sort=u.query.sort||'title';sql+=' ORDER BY '+(sort==='chapters'?'chapters DESC':sort==='author'?'s.author COLLATE NOCASE':sort==='new'?'s.updated_at DESC':'s.title COLLATE NOCASE');const page=Math.max(1,Number(u.query.page||1)),size=Math.max(1,Math.min(100,Number(u.query.pageSize||12)));const total=Number(q('SELECT COUNT(*) n FROM ('+sql+')',...args).n),pagedSql=sql+' LIMIT ? OFFSET ?',paged=all(pagedSql,...args,size,(page-1)*size),items=paged.map(storyRow);const out={items,page,total:Math.max(1,Math.ceil(total/size)),pageSize:size,count:total};cacheSet(ck,out);return reply(200,out)}
if(p.startsWith('/api/v1/admin/')){const u0=requireAdmin(req,res);if(!u0)return;
 ...
 if(p==='/api/v1/admin/stories'&&req.method==='GET')return reply(200,all(`SELECT s.*,COUNT(c.id) chapters FROM stories s LEFT JOIN chapters c ON c.story_id=s.id GROUP BY s.id ORDER BY s.updated_at DESC`).map(storyRow));
 ...
 if(p==='/api/v1/admin/stories/bulk'&&req.method==='PUT')return body(req).then(x=>{
  ...
  const t=now(); let n=0; db.exec('BEGIN'); try{for(const id of ids){if(x.category!=null){run('UPDATE stories SET category=?,search_key=?,updated_at=? WHERE id=?',String(x.category).slice(0,80),norm([getStory(id)?.title||'',getStory(id)?.author||'',x.category,getStory(id)?.description||''].join(' ')),t,id);n++} if(x.status!=null){run('UPDATE stories SET status=?,updated_at=? WHERE id=?',String(x.status).slice(0,30),t,id);n++}} db.exec('COMMIT');cacheClear('/api/v1/stories');reply(200,{ok:true,updated:n})}catch(e){db.exec('ROLLBACK');reply(400,{error:'BULK_UPDATE_FAILED',detail:e.message})}
...
if((m=p.match(/^\/api\/v1\/admin\/stories\/([^/]+)\/renumber$/))&&req.method==='POST')return body(req).then(x=>{
 ...
 const start=Math.max(1,Number(x.start||1));
...
function chapterizeText(text){
  const lines=String(text||'').replace(/^\uFEFF/,'').split(/\n/), re=/^\s*(?:chương|chuong|chapter|chap(?:ter)?|hồi|hoi)\s*(?:[.:#-]?\s*)?(\d+)?(?:\s*[:.\-–—]\s*|\s+)?(.{0,180})\s*$/i;let starts=[];
  lines.forEach((line,i)=>{const m=line.match(re);if(m&&((m[1]!==undefined)||/chương|chuong|chapter|hồi|hoi/i.test(line)))starts.push({line:i,num:m[1]?Number(m[1]):null,title:(m[2]||'').trim()})});
...
function parseEpub(buf){
  ...
  const title=(opf.match(/<dc:title[^>]*>([\s\S]*?)<\/dc:title>/i)||[])[1]||'';const author=(opf.match(/<dc:creator[^>]*>([\s\S]*?)<\/dc:creator>/i)||[])[1]||'';const manifest={};for(const m of opf.matchAll(/<item\b[^>]*id=["']([^"']+)["'][^>]*href=["']([^"']+)["'][^>]*media-type=["']([^"']+)["'][^>]*>/gi))manifest[m[1]]={href:m[2],type:m[3]};const spine=[];for(const m of opf.matchAll(/<itemref\b[^>]*idref=["']([^"']+)["'][^>]*>/gi)){if(manifest[m[1]])spine.push(manifest[m[1]])}
