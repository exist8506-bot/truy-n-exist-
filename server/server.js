       ...
       const rawIndex=Number(c.index??i),idx=Number.isInteger(rawIndex)&&rawIndex>=0?rawIndex:i;
        ...
        if(!present){
          run('INSERT INTO chapters(id,story_id,chapter_index,title,content,created_at,updated_at,search_key) VALUES(?,?,?,?,?,?,?,?)',
            cid,id,idx,String(c.title||('Chương '+(idx+1))),String(c.content||''),String(c.createdAt||t),String(c.updatedAt||t),norm([c.title||('Chương '+(idx+1)),c.content||''].join(' ')));
          ...
          if(!current?.content)run('UPDATE chapters SET title=?,content=?,updated_at=?,search_key=? WHERE id=?',
            String(c.title||('Chương '+(idx+1))),String(c.content||''),String(c.updatedAt||t),norm([c.title||('Chương '+(idx+1)),c.content||''].join(' ')),cid);
        }
      }
    }
    db.exec('COMMIT');
...
seed();
function rebuildChapterSearchKeys(){for(const r of all('SELECT id,title,content FROM chapters')){const key=norm([r.title||'',r.content||''].join(' '));if(q('SELECT search_key FROM chapters WHERE id=?',r.id)?.search_key!==key)run('UPDATE chapters SET search_key=? WHERE id=?',key,r.id)}}
rebuildChapterSearchKeys();
...
function prepareRestoreCovers(covers){
  if(covers==null)return[];
  if(typeof covers!=='object'||Array.isArray(covers))throw new Error('INVALID_BACKUP_COVERS');
  const safeName=/^[A-Za-z0-9_.-]{1,200}$/,out=[];let total=0;
  for(const [name,value] of Object.entries(covers)){
    if(!safeName.test(name))throw new Error('INVALID_BACKUP_COVER_NAME');
    const mm=String(value||'').match(/^data:image\/(png|jpeg|jpg|webp);base64,([A-Za-z0-9+/=]+)$/);
    if(!mm)throw new Error('INVALID_BACKUP_COVER');
    const buf=Buffer.from(mm[2],'base64');if(!buf.length)throw new Error('INVALID_BACKUP_COVER');
    total+=buf.length;if(buf.length>5*1024*1024)throw new Error('BACKUP_COVER_TOO_LARGE');if(total>25*1024*1024)throw new Error('BACKUP_COVERS_TOO_LARGE');
    out.push([path.basename(name),buf]);
  }
  return out;
}
...
m=p.match(/^\/api\/v1\/stories\/([^/]+)\/chapters$/);if(m&&req.method==='GET'){const ck=cacheKey(req),hit=cacheGet(ck);if(hit)return reply(200,hit);if(!getStory(m[1]))return reply(404,{error:'BOOK_NOT_FOUND'});const total=Number(q('SELECT COUNT(*) n FROM chapters WHERE story_id=?',m[1]).n),bounds=q('SELECT MIN(chapter_index) minIndex,MAX(chapter_index) maxIndex FROM chapters WHERE story_id=?',m[1]),minIndex=bounds?.minIndex==null?0:Number(bounds.minIndex),maxIndex=bounds?.maxIndex==null?-1:Number(bounds.maxIndex);const hasPaging=u.query.page!=null||u.query.pageSize!=null||u.query.q!=null;const cq=String(u.query.q||'').trim();if(!hasPaging)return reply(200,{items:all('SELECT id,story_id,chapter_index,title,updated_at FROM chapters WHERE story_id=? ORDER BY chapter_index',m[1]).map(chapterRow),minIndex,maxIndex,count:total});const page=clampedInteger(u.query.page,1,1,1000000),size=clampedInteger(u.query.pageSize,50,1,100),direction=String(u.query.sort||'asc').toLowerCase()==='desc'?'DESC':'ASC';let rows;if(cq){
 ...
 const targetIndex=isNumber?(minIndex===0?num-1:num):null;
...
if(p.startsWith('/api/v1/admin/')){const u0=requireAdmin(req,res);if(!u0)return;
 ...
 if(m&&req.method==='DELETE'){const sid=m[1];if(!getStory(sid))return reply(404,{error:'BOOK_NOT_FOUND'});let committed=false;db.exec('BEGIN');try{run('DELETE FROM bookmarks WHERE story_id=?',sid);run('DELETE FROM favorites WHERE story_id=?',sid);run('DELETE FROM reading_history WHERE story_id=?',sid);run('DELETE FROM reading_progress WHERE story_id=?',sid);run('DELETE FROM chapter_translations WHERE story_id=?',sid);run('DELETE FROM chapters WHERE story_id=?',sid);run('DELETE FROM stories WHERE id=?',sid);db.exec('COMMIT');committed=true;for(const f of fs.readdirSync(COVERS).filter(f=>f.startsWith(sid+'.')))try{fs.unlinkSync(path.join(COVERS,f))}catch{}cacheClear('/api/v1/stories');return reply(200,{ok:true})}catch(e){if(!committed){try{db.exec('ROLLBACK')}catch{}}return reply(committed?500:400,{error:'DELETE_STORY_FAILED',detail:e.message})}}
  m=p.match(/^\/api\/v1\/admin\/stories\/([^/]+)\/chapters$/);if(m&&req.method==='POST')return body(req).then(x=>{const s=getStory(m[1]);if(!s)return reply(404,{error:'BOOK_NOT_FOUND'});const i=x.index==null?Number(q('SELECT COALESCE(MAX(chapter_index),-1)+1 n FROM chapters WHERE story_id=?',s.id).n):Number(x.index);if(!Number.isInteger(i)||i<0)return reply(400,{error:'INVALID_CHAPTER_INDEX'});if(q('SELECT 1 FROM chapters WHERE story_id=? AND chapter_index=?',s.id,i))return reply(409,{error:'CHAPTER_EXISTS'});const content=String(x.content||'').trim();if(!content)return reply(400,{error:'INVALID_CHAPTER_CONTENT'});const id=`${s.id}:${i}`,t=now();run('INSERT INTO chapters(id,story_id,chapter_index,title,content,created_at,updated_at,search_key) VALUES(?,?,?,?,?,?,?,?)',id,s.id,i,String(x.title||`Chương ${i+1}`).slice(0,200),String(x.content||''),t,t,norm([String(x.title||`Chương ${i+1}`),String(x.content||'')].join(' ')));run('UPDATE stories SET updated_at=? WHERE id=?',t,s.id);cacheClear('/api/v1/stories');reply(201,chapterRow(q('SELECT * FROM chapters WHERE id=?',id)))});
 m=p.match(/^\/api\/v1\/admin\/stories\/([^/]+)\/chapters\/(\d+)$/);if(m&&req.method==='PUT')return body(req).then(x=>{const c=q('SELECT * FROM chapters WHERE story_id=? AND chapter_index=?',m[1],Number(m[2]));if(!c)return reply(404,{error:'CHAPTER_NOT_FOUND'});const content=String(x.content??c.content).trim();if(!content)return reply(400,{error:'INVALID_CHAPTER_CONTENT'});const t=now();run('UPDATE chapters SET title=?,content=?,updated_at=?,search_key=? WHERE id=?',String(x.title??c.title).slice(0,200),content,t,norm([String(x.title??c.title),content].join(' ')),c.id);run('DELETE FROM chapter_translations WHERE story_id=? AND chapter_index=?',m[1],Number(m[2]));run('UPDATE stories SET updated_at=? WHERE id=?',t,m[1]);cacheClear('/api/v1/stories');reply(200,chapterRow(q('SELECT * FROM chapters WHERE id=?',c.id)))});
 ...
 m=p.match(/^\/api\/v1\/admin\/stories\/([^/]+)\/cover$/);if(m&&req.method==='POST')return body(req,7*1024*1024).then(x=>{const s=getStory(m[1]);if(!s)return reply(404,{error:'BOOK_NOT_FOUND'});const data=String(x.data||''),mm=data.match(/^data:image\/(png|jpeg|jpg|webp);base64,(.+)$/);if(!mm)return reply(400,{error:'INVALID_IMAGE'});const ext=mm[1]==='jpeg'?'jpg':mm[1],name=`${s.id}.${ext}`,buf=Buffer.from(mm[2],'base64');if(buf.length>5*1024*1024)return reply(413,{error:'IMAGE_TOO_LARGE'});fs.writeFileSync(path.join(COVERS,name),buf);const rel='/covers/'+name;try{run('UPDATE stories SET cover_path=?,updated_at=? WHERE id=?',rel,now(),s.id)}catch(e){try{fs.unlinkSync(path.join(COVERS,name))}catch{}throw e}for(const f of fs.readdirSync(COVERS).filter(f=>f.startsWith(s.id+'.')&&f!==name))try{fs.unlinkSync(path.join(COVERS,f))}catch{}cacheClear('/api/v1/stories');reply(200,{cover:rel})}).catch(e=>reply(e.message==='BODY_TOO_LARGE'?413:400,{error:e.message||'BAD_JSON'}));
 ...
 if(p==='/api/v1/admin/import/commit'&&req.method==='POST')return body(req,25*1024*1024).then(x=>{try{const d=parseImportFile(String(x.filename||'truyen.txt'),x.data);if(!d.chapters.length||d.chapters.every(c=>!(String(c.content||'').trim())))return reply(400,{error:'NO_CHAPTERS'});const t=now(),id='b_'+token().slice(0,8),title=String(x.title||d.title||'Truyện nhập').slice(0,200),author=String(x.author||d.author||'').slice(0,120),cat=String(x.cat||'Khác').slice(0,80),desc=String(x.desc||'').slice(0,2000);const seen=new Set(),nums=[];const contentSeen=new Map(),empty=[],short=[],contentDuplicates=[];for(let ii=0;ii<d.chapters.length;ii++){const cc=d.chapters[ii],len=String(cc.content||'').trim().length;if(!len)empty.push(ii+1);else if(len<50)short.push(ii+1);const key=norm(cc.content||'').replace(/\s+/g,' ');if(key){if(contentSeen.has(key))contentDuplicates.push([contentSeen.get(key),ii+1]);else contentSeen.set(key,ii+1)}}if((empty.length||short.length||contentDuplicates.length)&&!x.allowIssues)return reply(400,{error:'IMPORT_ISSUES',issues:{duplicates:[],missing:[],empty,short,contentDuplicates},checked:d.chapters.length});for(const c of d.chapters){if(c.num!=null){if(seen.has(c.num)&&!x.allowIssues)return reply(400,{error:'DUPLICATE_CHAPTER_NUMBER',chapter:c.num});seen.add(c.num);nums.push(c.num)}}if(nums.length&&!x.allowIssues){const min=Math.min(...nums),max=Math.max(...nums),missing=[];for(let n=min;n<=max;n++)if(!seen.has(n))missing.push(n);if(missing.length)return reply(400,{error:'MISSING_CHAPTER_NUMBER',chapters:missing.slice(0,50)})}let committed=false;db.exec('BEGIN IMMEDIATE');run('INSERT INTO stories VALUES(?,?,?,?,?,?,?,?,?,?,?)',id,title,author,cat,desc,norm([title,author,cat,desc].join(' ')),String(x.tone||'').slice(0,200),'FULL','',t,t);for(let i=0;i<d.chapters.length;i++){const c=d.chapters[i];run('INSERT INTO chapters(id,story_id,chapter_index,title,content,created_at,updated_at,search_key) VALUES(?,?,?,?,?,?,?,?)',`${id}:${i}`,id,i,String(c.title||`Chương ${i+1}`).slice(0,200),c.content||'',t,t,norm([String(c.title||`Chương ${i+1}`),String(c.content||'')].join(' ')))}run('UPDATE stories SET updated_at=? WHERE id=?',now(),id);db.exec('COMMIT');committed=true;cacheClear('/api/v1/stories'); reply(201,{ok:true,story:storyRow(getStory(id)),imported:d.chapters.length,format:d.format})}catch(e){if(!committed){try{db.exec('ROLLBACK')}catch{}}reply(committed?500:400,{error:'IMPORT_COMMIT_FAILED',detail:e.message})}}).catch(e=>reply(e.message==='BODY_TOO_LARGE'?413:400,{error:e.message||'BAD_JSON'}));
...
if(p==='/api/v1/admin/import/batch'&&req.method==='POST')return body(req,30*1024*1024).then(x=>{
  ...
  try{
    ...
    if((issues.duplicates.length||issues.missing.length||issues.empty.length||issues.short.length||issues.contentDuplicates.length)&&!x.allowIssues){run('UPDATE import_jobs SET status=?,error=?,updated_at=? WHERE id=?','REJECTED','IMPORT_ISSUES',now(),jobId);return reply(400,{error:'IMPORT_ISSUES',issues,checked:chapters.length,jobId});}
     let committed=false;db.exec('BEGIN IMMEDIATE');
    try{
      ...
      for(let start=0;start<chapters.length;start+=batchSize){
        for(let i=start;i<Math.min(start+batchSize,chapters.length);i++){const c=chapters[i];run('INSERT INTO chapters(id,story_id,chapter_index,title,content,created_at,updated_at,search_key) VALUES(?,?,?,?,?,?,?,?)',`${id}:${i}`,id,i,String(c.title||`Chương ${i+1}`).slice(0,200),c.content||'',t,t,norm([String(c.title||`Chương ${i+1}`),String(c.content||'')].join(' ')));done++;}
        run('UPDATE import_jobs SET processed=?,updated_at=? WHERE id=?',done,now(),jobId);}
      run('UPDATE stories SET updated_at=? WHERE id=?',now(),id); db.exec('COMMIT');committed=true; run('UPDATE import_jobs SET status=?,processed=?,updated_at=? WHERE id=?','COMPLETED',done,now(),jobId);
      cacheClear('/api/v1/stories'); return reply(201,{ok:true,jobId,story:storyRow(getStory(id)),imported:done,batchSize,issues});
     }catch(e){if(!committed){try{db.exec('ROLLBACK')}catch{};run('UPDATE import_jobs SET status=?,error=?,updated_at=? WHERE id=?','FAILED',String(e.message||'IMPORT_ROLLBACK').slice(0,500),now(),jobId);return reply(400,{error:'IMPORT_ROLLBACK',detail:e.message,jobId})}return reply(500,{error:'IMPORT_POST_COMMIT_FAILED',detail:e.message,jobId})}
...
if(p==='/api/v1/admin/backup'&&req.method==='GET'){const covers={};for(const f of fs.readdirSync(COVERS)){const fp=path.join(COVERS,f);if(fs.statSync(fp).isFile())covers[f]='data:'+mime(fp).split(';')[0]+';base64,'+fs.readFileSync(fp).toString('base64')}const snap={version:'1.6.1',createdAt:now(),users:all('SELECT * FROM users'),profiles:all('SELECT * FROM profiles'),stories:all('SELECT * FROM stories'),chapters:all('SELECT * FROM chapters'),progress:all('SELECT * FROM reading_progress'),history:all('SELECT * FROM reading_history'),favorites:all('SELECT * FROM favorites'),bookmarks:all('SELECT * FROM bookmarks'),covers};return reply(200,snap)}
 if(p==='/api/v1/admin/backup/restore'&&req.method==='POST')return body(req,30*1024*1024).then(x=>{if(!Array.isArray(x.stories)||!Array.isArray(x.chapters)||!Array.isArray(x.users))return reply(400,{error:'INVALID_BACKUP'});const safeId=/^[A-Za-z0-9_.:-]{1,128}$/,invalidIds=[...x.users,...x.stories,...x.chapters].filter(r=>r&&((r.id!=null&&!safeId.test(String(r.id)))||(r.story_id!=null&&!safeId.test(String(r.story_id)))||(r.user_id!=null&&!safeId.test(String(r.user_id)))));if(invalidIds.length)return reply(400,{error:'INVALID_BACKUP_IDS',count:invalidIds.length});const userIds=new Set(x.users.map(r=>String(r?.id||''))),storyIds=new Set(x.stories.map(r=>String(r?.id||''))),chapterKeys=new Set(x.chapters.map(r=>String(r?.story_id||'')+':'+String(r?.chapter_index??''))),invalid=[];for(const r of x.chapters)if(!storyIds.has(String(r?.story_id||''))||!Number.isInteger(Number(r?.chapter_index))||Number(r.chapter_index)<0)invalid.push({type:'chapter',storyId:r?.story_id,chapterIndex:r?.chapter_index});for(const r of (x.progress||[]).concat(x.history||[])){const key=String(r?.story_id||'')+':'+String(r?.chapter_index??'');if(!userIds.has(String(r?.user_id||''))||!storyIds.has(String(r?.story_id||''))||!chapterKeys.has(key)||!Number.isInteger(Number(r?.chapter_index))||!Number.isFinite(Number(r?.position))||Number(r.position)<0||Number(r.position)>100)invalid.push({type:'progress_or_history',userId:r?.user_id,storyId:r?.story_id,chapterIndex:r?.chapter_index})}for(const r of (x.bookmarks||[])){const key=String(r?.story_id||'')+':'+String(r?.chapter_index??'');if(!userIds.has(String(r?.user_id||''))||!storyIds.has(String(r?.story_id||''))||!chapterKeys.has(key)||!Number.isInteger(Number(r?.chapter_index)))invalid.push({type:'bookmark',userId:r?.user_id,storyId:r?.story_id,chapterIndex:r?.chapter_index})}for(const r of (x.favorites||[]))if(!userIds.has(String(r?.user_id||''))||!storyIds.has(String(r?.story_id||'')))invalid.push({type:'favorite',userId:r?.user_id,storyId:r?.story_id});for(const r of (x.profiles||[]))if(!userIds.has(String(r?.user_id||'')))invalid.push({type:'profile',userId:r?.user_id});if(invalid.length)return reply(400,{error:'INVALID_BACKUP_REFERENCES',count:invalid.length,items:invalid.slice(0,50)});let restoreCovers;try{restoreCovers=prepareRestoreCovers(x.covers)}catch(e){return reply(400,{error:String(e.message||'INVALID_BACKUP_COVERS')})}let committed=false;db.exec('BEGIN');try{db.exec('DELETE FROM chapter_translations; DELETE FROM bookmarks; DELETE FROM favorites; DELETE FROM reading_history; DELETE FROM reading_progress; DELETE FROM chapters; DELETE FROM stories; DELETE FROM profiles; DELETE FROM sessions; DELETE FROM users;');for(const r of x.users)run('INSERT INTO users VALUES(?,?,?,?,?,?,?)',r.id,r.username,r.password_hash,r.password_salt,r.display_name,r.role,r.created_at);for(const r of (x.profiles||[]))run('INSERT INTO profiles VALUES(?,?,?)',r.user_id,r.avatar||'',r.updated_at||now());for(const r of x.stories)run('INSERT INTO stories VALUES(?,?,?,?,?,?,?,?,?,?,?)',r.id,r.title,r.author,r.category,r.description,norm([r.title,r.author,r.category,r.description].join(' ')),r.tone,r.status,r.cover_path||'',r.created_at,r.updated_at);for(const r of x.chapters)run('INSERT INTO chapters(id,story_id,chapter_index,title,content,created_at,updated_at,search_key) VALUES(?,?,?,?,?,?,?,?)',r.id,r.story_id,r.chapter_index,r.title,r.content,r.created_at,r.updated_at,norm([r.title||'',r.content||''].join(' ')));for(const r of (x.progress||[]))run('INSERT INTO reading_progress VALUES(?,?,?,?,?)',r.user_id,r.story_id,r.chapter_index,r.position,r.updated_at);for(const r of (x.history||[]))run('INSERT INTO reading_history VALUES(?,?,?,?,?)',r.user_id,r.story_id,r.chapter_index,r.position,r.updated_at);for(const r of (x.favorites||[]))run('INSERT INTO favorites VALUES(?,?,?)',r.user_id,r.story_id,r.created_at);for(const r of (x.bookmarks||[]))run('INSERT INTO bookmarks VALUES(?,?,?,?,?,?)',r.user_id,r.story_id,r.chapter_index,r.title||'',r.created_at,r.updated_at);db.exec('COMMIT');committed=true;cacheClear('/api/v1/stories');for(const f of fs.readdirSync(COVERS))fs.unlinkSync(path.join(COVERS,f));for(const [name,buf] of restoreCovers)fs.writeFileSync(path.join(COVERS,name),buf);const restoredAdmin=q('SELECT * FROM users WHERE id=? AND role=?',u0.id,'admin');let restoreToken=null;
if(restoredAdmin){restoreToken=token();run('INSERT INTO sessions VALUES(?,?,?,?)',restoreToken,u0.id,new Date(Date.now()+SESSION_DAYS*864e5).toISOString(),now());}
reply(200,{ok:true,token:restoreToken})}catch(e){if(!committed){try{db.exec('ROLLBACK')}catch{}}reply(committed?500:400,{error:'RESTORE_FAILED',detail:e.message})}}).catch(()=>reply(400,{error:'BAD_JSON'}));
}
...
function zipRead(buf,e){const p=e.lo;if(e.usize>25*1024*1024)throw new Error('ZIP_ENTRY_TOO_LARGE');if(e.csize>buf.length||p<0||p+30+e.csize>buf.length)throw new Error('INVALID_ZIP_ENTRY');if(buf.readUInt32LE(p)!==0x04034b50)throw new Error('INVALID_ZIP_LOCAL');const nl=buf.readUInt16LE(p+26),el=buf.readUInt16LE(p+28),start=p+30+nl+el;if(start<0||start+e.csize>buf.length)throw new Error('INVALID_ZIP_ENTRY');const data=buf.slice(start,start+e.csize);if(e.method===0)return data;if(e.method===8)return zlib.inflateRawSync(data,{maxOutputLength:25*1024*1024});throw new Error('UNSUPPORTED_ZIP_COMPRESSION')}
...
function parseEpub(buf){
  ...
  const title=(opf.match(/<dc:title[^>]*>([\s\S]*?)<\/dc:title>/i)||[])[1]||'';const author=(opf.match(/<dc:creator[^>]*>([\s\S]*?)<\/dc:creator>/i)||[])[1]||'';const manifest={};for(const m of opf.matchAll(/<item\b[^>]*>/gi)){const tag=m[0],attrs={};for(const a of tag.matchAll(/([:\w-]+)\s*=\s*["']([^"']*)["']/g))attrs[a[1]]=a[2];if(attrs.id&&attrs.href&&attrs['media-type'])manifest[attrs.id]={href:attrs.href,type:attrs['media-type']}}const spine=[];for(const m of opf.matchAll(/<itemref\b[^>]*>/gi)){const attrs={};for(const a of m[0].matchAll(/([:\w-]+)\s*=\s*["']([^"']*)["']/g))attrs[a[1]]=a[2];if(attrs.idref&&manifest[attrs.idref])spine.push(manifest[attrs.idref])}
  const chapters=[];for(const it of spine){if(!/html|xhtml/i.test(it.type))continue;let href=String(it.href||'').split('#')[0];try{href=decodeURIComponent(href)}catch{}const name=path.posix.normalize(path.posix.join(base,href));const raw=readName(name);if(!raw)continue;const txt=stripHtml(raw);if(!txt)continue;const local=chapterizeText(txt);for(const c of local)chapters.push(c)}return {title:stripHtml(title),author:stripHtml(author),chapters:chapters.map((c,i)=>({...c,index:i,title:c.title||`Chương ${i+1}`}))};
}
