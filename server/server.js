...
if(p.startsWith('/api/v1/admin/')){const u0=requireAdmin(req,res);if(!u0)return;
 ...
 if(m&&req.method==='DELETE'){const sid=m[1];if(!getStory(sid))return reply(404,{error:'BOOK_NOT_FOUND'});let committed=false;db.exec('BEGIN');try{run('DELETE FROM bookmarks WHERE story_id=?',sid);run('DELETE FROM favorites WHERE story_id=?',sid);run('DELETE FROM reading_history WHERE story_id=?',sid);run('DELETE FROM reading_progress WHERE story_id=?',sid);run('DELETE FROM chapter_translations WHERE story_id=?',sid);run('DELETE FROM chapters WHERE story_id=?',sid);run('DELETE FROM stories WHERE id=?',sid);db.exec('COMMIT');committed=true;for(const f of fs.readdirSync(COVERS).filter(f=>f.startsWith(sid+'.')))try{fs.unlinkSync(path.join(COVERS,f))}catch{}cacheClear('/api/v1/stories');return reply(200,{ok:true})}catch(e){if(!committed){try{db.exec('ROLLBACK')}catch{}}return reply(committed?500:400,{error:'DELETE_STORY_FAILED',detail:e.message})}}
  m=p.match(/^\/api\/v1\/admin\/stories\/([^/]+)\/chapters$/);if(m&&req.method==='POST')return body(req).then(x=>{const s=getStory(m[1]);if(!s)return reply(404,{error:'BOOK_NOT_FOUND'});const i=x.index==null?Number(q('SELECT COALESCE(MAX(chapter_index),-1)+1 n FROM chapters WHERE story_id=?',s.id).n):Number(x.index);if(!Number.isInteger(i)||i<0)return reply(400,{error:'INVALID_CHAPTER_INDEX'});if(q('SELECT 1 FROM chapters WHERE story_id=? AND chapter_index=?',s.id,i))return reply(409,{error:'CHAPTER_EXISTS'});const content=content.trim();if(!content)return reply(400,{error:'INVALID_CHAPTER_CONTENT'});const id=`${s.id}:${i}`,t=now();run('INSERT INTO chapters(id,story_id,chapter_index,title,content,created_at,updated_at,search_key) VALUES(?,?,?,?,?,?,?,?)',id,s.id,i,String(x.title||`Chương ${i+1}`).slice(0,200),String(x.content||''),t,t,norm([String(x.title||`Chương ${i+1}`),String(x.content||'')].join(' ')));run('UPDATE stories SET updated_at=? WHERE id=?',t,s.id);cacheClear('/api/v1/stories');reply(201,chapterRow(q('SELECT * FROM chapters WHERE id=?',id)))});
